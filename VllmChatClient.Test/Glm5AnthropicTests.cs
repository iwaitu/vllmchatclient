...
namespace VllmChatClient.Test
{
    ...
    public class Glm5AnthropicTests
    {
        ...
        public Glm5AnthropicTests(ITestOutputHelper output)
        {
            ...
            _client = new VllmGlmChatClient(
                "https://dashscope.aliyuncs.com/apps/anthropic",
                cloud_apiKey,
                MODEL,
                null,
                VllmApiMode.AnthropicMessages);
            _chatOptions = new GlmChatOptions { ThinkingEnabled = true, MaxOutputTokens = 3000 };
        ...
        public async Task TestJsonSchemaOutput()
        {
            ...
            {
                new ChatMessage(ChatRole.System ,"你是一个智能助手，名字叫菲菲"),
                new ChatMessage(ChatRole.User,$"请严格按以下 JSON schema 返回 JSON 对象。输出必须且只能包含 name 和 greeting 两个字符串字段，其中 name 必须是“菲菲”，greeting 必须是一句问候语。不要输出代码块，也不要输出 JSON 之外的任何文字。\n\nJSON Schema:\n{schema.GetRawText()}")
            };
        ...
        public async Task StreamFunctionCallThenJsonSchemaOutputTest()
        {
            ...
            var messages = new List<ChatMessage>
            {
                ...
                new ChatMessage(ChatRole.User, $"请查询南宁火车站在哪里，并严格按以下 JSON schema 返回。summary 里请简要说明这是通过工具查询到的结果。\n\nJSON Schema:\n{schema.GetRawText()}")
            };
